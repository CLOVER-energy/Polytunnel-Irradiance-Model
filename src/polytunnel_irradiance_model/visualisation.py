...
def plot_surface(
    X, Y, Z, normals_unit, X_ground, Y_ground, Z_ground, normals_unit_ground, sun_vec
):
    ...
    ax.plot_surface(
        X_ground, Y_ground, Z_ground, color="brown", alpha=0.6
    )  # Ground surface
...
def plot_irradiance(ground_grid_x, ground_grid_y, irradiance_array):
    ...
    contour = ax.contourf(
        ground_grid_x, ground_grid_y, irradiance_array, levels=100, cmap="hot"
    )
...
def animate_irradiance(
    time_array, ground_grid_x, ground_grid_y, irradiance_array, label_colorbar, filename
):
    # Create a figure and axis
    fig, ax = plt.subplots(figsize=(10, 5))
    ...
    list_index = next(
        i for i, sublist in enumerate(irradiance_array) if irradiance_max in sublist
    )
    ...
    anim = FuncAnimation(
        fig, update, frames=len(irradiance_array), interval=50, blit=True
    )
...
def plot_power(
    time_array, power_ground_diffuse, power_ground_direct, power_ground, filename
):
    ...
    plt.stackplot(
        time_array,
        power_ground_direct,
        power_ground_diffuse,
        labels=["Ground (Direct)", "Ground (Diffuse)"],
    )
    # Overlay the global power plot
    plt.plot(
        time_array, power_ground, label="Ground (Global)", color="black", linestyle="--"
    )
...
def every_grid_plot(
    spectra_frames, time_array, optical_wavelengths, res_minutes, filename
):
    ...
    fig, ax = plt.subplots(
        sides_tiles, length_tiles, figsize=(4 * length_tiles, 4 * sides_tiles)
    )
    max_val = 0
    ...
    for i in range(n_frames):
        ...
        for q in range(sides_tiles):  #
            ...
            for p in range(length_tiles):  #
                ...
                ax[q, p].plot(
                    optical_wavelengths, spectra_frames[i][q][p], color=colors[i]
                )  # Curva de cada frame
    ...
    cticks = [item * (60 // res_minutes) * 3 for item in range(8)] + [
        len(time_array) - 1
    ]
